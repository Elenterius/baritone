        ...
        if (Baritone.settings().miningCheckInventory.value) {
            ...
            for (ItemStack stack : invy) {
                ...
                for (Block block : mining) {
                    miningDrops.add(block.getItemDropped(mining.get(0).getDefaultState(), ctx.world(), new BlockPos(0, 0, 0), 0).asItem());
                }
            ...
            if (inventoryFull && !DropsHaveSpace.isEmpty()) {
                inventoryFull = false;
                for (Block block : mining) {
                    if (!DropsHaveSpace.contains(block.getItemDropped(mining.get(0).getDefaultState(), ctx.world(), new BlockPos(0, 0, 0), 0).asItem())) {
                        mining.remove(block);
                    }
                }
